# IDS-008 · Design an E-commerce Platform
...
Design a multi-seller marketplace with catalog, search, cart, checkout, inventory reservation, payment, order fulfillment, returns, reviews, and notifications. Assume 50 million monthly buyers, 5 million products, 100,000 sellers, 100,000 peak browse requests/second, 10,000 checkout attempts/second, and flash-sale hot keys.

### Bounded contexts

Catalog owns product facts and seller offers; search owns a denormalized index; cart owns short-lived intent; inventory owns sellable quantity; order owns the durable purchase state; payment, tax, shipping, and returns are external or separate services. Checkout creates an order intent, reserves inventory with a lease, authorizes payment, and commits an order through a saga. Never use a search index or cart cache as the source of truth for stock or money.

### APIs and model

`GET /v1/products`, `PUT /v1/carts/{id}/items`, `POST /v1/checkout`, `GET /v1/orders/{id}`, and seller fulfillment endpoints are versioned and idempotent. Tables include `offers(seller_id, sku, price, version)`, `inventory(sku, location, on_hand, reserved, version)`, `orders(order_id, buyer_id, total, state, version)`, `order_lines`, `payments`, and `outbox_events`. Index buyer orders by `(buyer_id, created_at, order_id)`, stock by `(sku, location)`, and use immutable money/currency snapshots on order lines.

### Correctness and flash sales

Inventory invariant: `on_hand - reserved >= 0`; reserve atomically with a version check or a single-writer partition. A reservation has an expiry and release event; payment retries reuse the same authorization key. Order transitions are monotonic (`PENDING -> PAID -> FULFILLING -> SHIPPED -> DELIVERED`, with explicit cancel/return branches). For flash sales, pre-allocate quotas by shard, queue excess demand, and communicate eventual confirmation rather than allowing an unbounded database hotspot.

### Security, operations, and evolution

Tokenize payment data, isolate seller tenants, enforce price/tax auditability, scan seller content, and rate-limit coupon/checkout abuse. Observe cart-to-order conversion, reservation expiry, payment failure, inventory drift, seller SLA, queue lag, and p99 checkout latency. Discuss marketplace versus first-party ownership, split shipments, partial refunds, personalization, and multi-region residency.
