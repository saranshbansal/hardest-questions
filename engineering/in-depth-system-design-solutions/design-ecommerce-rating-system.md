# IDS-016 · Design a rating system for an e-commerce website
...
Design product ratings/reviews with verified purchase signals, stars, text/media, helpful votes, moderation, seller responses, edits, deletion, aggregation, and fraud controls. Assume 100 million products, 50 million new reviews/month, 2 billion rating reads/month, 200,000 writes/second during campaigns, and a 5-minute aggregate freshness target.

### Architecture and moderation

Review service stores immutable revisions and a current pointer; order/entitlement service supplies a verified-purchase fact without exposing order data to readers. Media is scanned and served via CDN. Moderation combines synchronous policy checks with an asynchronous classifier and human queue. Aggregation consumes accepted review events into per-product star histograms, Bayesian/weighted scores, and sampled text summaries. A product page reads the aggregate projection and paginates reviews from a search/read store.

### APIs and data

`POST /v1/products/{id}/reviews`, `PATCH /v1/reviews/{id}`, `POST /v1/reviews/{id}/helpful`, `POST /v1/reviews/{id}/report`, and `GET /v1/products/{id}/ratings` use idempotency and authorization. Tables: `reviews(review_id, product_id, author_id, state, rating, revision, verified, created_at)`, `review_revisions`, `review_votes(review_id, user_id)`, `moderation_cases`, and `rating_buckets(product_id, star, count, version)`. Unique active review policy can be `(product_id, author_id, order_line_id)`; vote uniqueness is `(review_id, user_id)`.

### Correctness, fairness, and abuse

Only an accepted revision contributes to aggregates; moderation decisions are versioned and replayable. Helpful votes are idempotent and rate-limited. Never allow seller access to alter a customer review; seller responses are separate records. Detect brigading, review exchanges, bot clusters, duplicate text, incentive abuse, and retaliation. Show verified and incentivized labels, confidence/sample size, and a clear appeal path. Deletion propagates through search/cache/aggregate within a stated bound and preserves an audit record where legally required.

### Aggregation and follow-ups

A simple mean is manipulable; expose histogram and use Bayesian shrinkage toward a prior for ranking, while showing transparent star counts. Cache product aggregates with event-driven invalidation and bounded staleness. Monitor moderation backlog, false-positive appeals, aggregate lag, vote abuse, cache hit rate, and review-read latency. Discuss international content law, anonymous reviews, multilingual toxicity, seller-level ratings, and whether ratings should influence search ranking.
