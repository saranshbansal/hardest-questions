# IDS-006 · Design Google Play Store
...
Design an app marketplace: developer publishing, review/approval, catalog search, versioned delivery, staged rollout, installation/update, ratings, payments as an external boundary, and device compatibility. Assume 3 billion registered devices, 100 million daily catalog queries, 20 million daily downloads, 500,000 developers, and 10,000 publishing actions/hour. A release must be immutable after approval and downloadable from a nearby CDN.

### Services and release lifecycle

Use developer identity, package registry, malware scanning, policy review, catalog/search, entitlement, rollout, delivery, and device-install services. Uploads go to object storage, are hashed and signed, scanned in isolated sandboxes, and become immutable artifacts. A release state machine is `draft -> uploaded -> scanning -> review -> approved -> staged -> full -> deprecated/recalled`. The catalog is a projection from approved metadata; the package registry is the source of truth.

### APIs and data

`POST /v1/apps/{id}/releases`, `POST /v1/releases/{id}/submit`, `GET /v1/catalog/search`, `GET /v1/apps/{id}/compatible-artifacts`, and `POST /v1/installations` use signed device/developer tokens. Model `apps`, `releases(app_id, version, artifact_digest, status)`, `device_capabilities(device_id, os, abi, country)`, `entitlements(account_id, app_id, rights)`, and `rollout_cohorts`. Unique `(app_id, version)` and digest; index catalog text separately from transactional metadata.

### Delivery correctness and safety

A client downloads a manifest containing digest, signature, compatibility constraints, and rollout decision, then verifies before install. Entitlement checks are strongly consistent enough to prevent unauthorized paid access; catalog and rating views are eventually consistent. Rollouts use cohorts, health signals, automatic halt thresholds, and signed rollback manifests. Abuse defenses cover malware, fake reviews, developer account takeover, scraper traffic, and copyright/takedown processes. Protect user privacy by minimizing install telemetry and offering regional retention controls.

### Operations and evolution

CDN multi-region caching and resumable downloads handle scale; origin uses immutable content addressing. Monitor scan backlog, approval age, CDN hit ratio, failed installs, crash rate by artifact, rollout blast radius, and recall propagation. Discuss payment ledger boundaries, parental controls, offline installs, anti-fraud, and compatibility with old devices.
