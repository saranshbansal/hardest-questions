# IDS-012 · Design Library management system LLD
...
Design a library domain with members, books, physical copies, holds, loans, renewals, returns, fines, and notifications. Keep catalog metadata separate from copy inventory. Interfaces isolate persistence, clock, payment, and messaging so deterministic tests can cover policy edge cases.

### Domain model

```text
Book(isbn, title, authors, subjects)
BookCopy(copyId, isbn, branchId, status)
Member(memberId, status, loanLimit)
Loan(loanId, copyId, memberId, dueAt, state, version)
Hold(holdId, isbn, memberId, queuePosition, state)
Fine(fineId, memberId, amount, state)

Catalog.search(query, cursor)
Circulation.checkout(memberId, copyId, idempotencyKey)
Circulation.returnCopy(loanId, idempotencyKey)
Policy.canRenew(member, loan, now)
```

### State machines and concurrency

Copy states are `AVAILABLE -> ON_LOAN -> AVAILABLE`, with `ON_HOLD`, `LOST`, and `REPAIR` branches. A hold is `QUEUED -> READY -> FULFILLED/EXPIRED/CANCELLED`; a loan is `OPEN -> OVERDUE -> RETURNED`. Checkout runs in one transaction: lock the copy, confirm member eligibility and no conflicting hold, insert loan, change copy state, and record the command key. Two branches cannot check out one copy. A renewal uses optimistic versioning and rechecks a waiting hold; return is idempotent and atomically advances the next hold.

```text
checkout(cmd):
  if commands.exists(cmd.key): return commands.result(cmd.key)
  tx:
    copy = copies.lock(cmd.copyId)
    member = members.lock(cmd.memberId)
    assert copy.status == AVAILABLE
    assert policy.canBorrow(member, copy)
    loan = loans.insert(copy, member, dueAt=clock.now()+policy.term)
    copies.setStatus(copy, ON_LOAN)
    commands.record(cmd.key, loan.id)
  notifier.enqueue(LoanCreated(loan.id))
  return loan
```

### Persistence, API, and testing

Use `POST /v1/loans`, `POST /v1/loans/{id}/return`, `POST /v1/holds`, and `POST /v1/loans/{id}/renew` with idempotency keys. Unique active loan per copy and active hold per member/ISBN are database constraints. A daily job marks overdue loans and accrues fines; it is restartable and uses a watermark. Test concurrent checkout, duplicate return, lost card, branch transfer, clock boundaries, notification failure, and fine payment retry. Ask how digital licenses, inter-library loans, and privacy deletion change the design.
