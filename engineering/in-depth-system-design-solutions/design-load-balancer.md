# IDS-003 · Design a Load Balancer
...
Design a layer-4/7 service for HTTP and TCP workloads, including health checks, TLS termination, routing policies, connection draining, and multi-AZ failover. Assume 5 million active connections, 1 million new connections/second at peak, 20 Gbit/s ingress, and 50,000 backend targets across 200 pools. Require p99 forwarding overhead below 5 ms and no new connections to an unhealthy target within one health interval.

### Planes and packet path

Keep a globally replicated control plane (listeners, certificates, pools, policy versions) separate from regional data-plane load balancers. Anycast/DNS directs clients to a healthy region; regional nodes use consistent hashing or least-loaded selection. L4 uses kernel/eBPF or specialized datapath forwarding; L7 parses HTTP, enforces policy, and can route by host/path/header. Config snapshots are immutable and atomically swapped into dataplane workers.

### Health, draining, and correctness

Active checks test the actual dependency path with timeouts and consecutive failure/success thresholds; passive checks watch connection resets and elevated 5xx. A target transitions `healthy -> suspect -> draining -> unhealthy`, with hysteresis to prevent flapping. A config epoch and target lease fence stale controllers. Connection draining honors a deadline, sends GOAWAY for HTTP/2, and never drops an acknowledged request. Retries are disabled for non-idempotent requests unless the application supplies an idempotency key.

### APIs and capacity

`PUT /v1/listeners/{id}/config` accepts a versioned desired state; `POST /v1/pools/{id}/targets`; `GET /v1/health`. Store desired state in a strongly consistent control database and publish snapshots through a signed stream. Metrics include active connections, new-connection rate, backend selection skew, health transitions, dropped packets, handshake latency, config convergence, and zone imbalance. Use per-tenant quotas, SYN cookies, connection caps, request-body limits, WAF/rate-limit stages, and DDoS scrubbing upstream.

### Failure and evolution

If control-plane connectivity fails, data-plane nodes continue the last valid snapshot for a bounded TTL, then fail safe according to endpoint risk. On AZ loss, withdraw addresses, rebalance to remaining capacity, and protect the backends with admission control. Start with round-robin/least-connections; add weighted, locality-aware, and latency-aware routing only with measurement. Hardware offload can reduce CPU but complicates portability and debugging.
