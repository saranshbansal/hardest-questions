# IDS-014 · Design a reservation system for a parking lot
...
Design reservations for parking facilities with search, pricing, entry/exit, permits, cancellations, payments, and live occupancy. Assume 10,000 facilities, 2 million spaces, 5 million reservations/day, 20,000 peak reservation attempts/second, and occasional event surges. A reservation must not promise the same space/time interval twice.

### Architecture and allocation

Facility metadata and pricing are read-optimized; an availability service maintains interval/bucket capacity by facility, zone, vehicle class, and time. A reservation service atomically claims capacity, then a payment saga confirms it. Gate controllers use signed reservation tokens and an occupancy event stream; they do not directly edit the source of truth. For facilities with interchangeable spaces, reserve a capacity bucket and assign a concrete space at entry; for accessible/EV spaces, use class-specific pools and policy checks.

### APIs and model

`GET /v1/facilities/search`, `POST /v1/reservations/quote`, `POST /v1/reservations`, `POST /v1/reservations/{id}/cancel`, and `POST /v1/gates/events` are versioned. Tables: `facilities`, `spaces`, `capacity_buckets(facility, class, start_bucket, total, held, confirmed)`, `reservations`, `payments`, and `gate_events`. Unique reservation command keys and a capacity version protect retries; indexes support `(facility_id, start_time)` and active reservations by token.

### Correctness and failure handling

The invariant is `confirmed + held <= total`, with holds expiring automatically. A reservation is `HELD -> CONFIRMED -> CHECKED_IN -> CHECKED_OUT`; cancellation/refund policy is explicit. Gate events are signed, deduplicated, and reconciled against occupancy; a lost gate connection uses a bounded local allowlist and later sync. If payment succeeds but confirmation times out, query by idempotency key before compensating. Prevent token replay, tailgating abuse, and overstay with short-lived tokens and anomaly rules.

### Scale and trade-offs

Partition by facility and date; cache search results but revalidate capacity at hold time. Monitor hold conversion, oversell attempts, gate decision latency, occupancy drift, payment reconciliation, and event-surge queue age. Compare exact interval locking with bucketed capacity: buckets scale and fit interchangeable spaces but may reduce utilization at boundaries. Discuss valet assignment, reservations across time zones, EV charging, and no-show policy.
