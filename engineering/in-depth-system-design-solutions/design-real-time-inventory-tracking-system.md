# IDS-015 · Design a Real-time Inventory Tracking System
...
Design a real-time inventory tracker combining warehouse scans, point-of-sale sales, transfers, returns, supplier receipts, and IoT signals. Assume 50 million SKUs, 500,000 events/second peak, 10 billion ledger events retained, p95 availability freshness under 5 seconds, and 99.99% durable ingestion.

### Event-first architecture

Sources emit signed, schema-versioned `InventoryEvent`s to regional collectors. A partitioned stream keyed by inventory identity feeds a ledger writer, balance projector, reservation service, anomaly detector, and query API. The append-only ledger is authoritative; balances are projections with a checkpoint and checksum. Cross-region replication preserves the event ID and source sequence; consumers can rebuild from object-storage archives.

### Contracts and data model

Event fields: `event_id`, `source_id`, `source_sequence`, `sku`, `owner`, `location`, `quantity_delta`, `unit`, `lot/serial`, `reason`, `occurred_at`, and `schema_version`. Reject ambiguous units and impossible transitions at ingress. Tables/streams include `inventory_events`, `balances(sku, location, lot, on_hand, reserved, version)`, `reservations`, `consumer_checkpoints`, and `reconciliation_cases`. Dedup by `(source_id, source_sequence)` and globally by event ID; partition by `(owner, location, sku)` to preserve local order.

### Correctness and reconciliation

The balance equals the sum of accepted ledger deltas minus reservations and adjustments. A late event is applied by event time only in analytics; operational balance uses arrival order plus a correction record to avoid rewriting history. Reservation is an atomic conditional decrement of available balance. Consumers checkpoint after durable application; replay is safe. Periodic physical counts and source snapshots detect drift, while repair requires an adjustment reason and approval.

### Resilience, privacy, and operations

Edge devices buffer encrypted batches with sequence numbers and bounded disk. Backpressure or `429` protects the ledger; never silently drop a stock event. Monitor ingestion durability, end-to-end freshness, consumer lag, duplicate/rejected rates, negative availability, drift, and hot partitions. Discuss exactly-once claims (usually an outcome of idempotent at-least-once processing), serial tracking, multi-owner stock, and eventual consistency exposed to customers.
