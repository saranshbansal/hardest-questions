...
Design a public microblogging service: accounts, follows, text/media posts, home timeline, likes, replies, search, notifications, and deletion. Exclude private messaging and ad ranking from the first pass; mention them as separate bounded contexts. Assume 250 million daily active users, 500 million posts/day, 40% of reads are home timelines, 2 million peak timeline reads/second, and a 30-day hot timeline window.

### Service layout

Use identity/profile, graph, post/media, timeline, engagement, search, and notification services. Object storage plus a CDN serves media. A follow edge is stored in a graph store, while posts are immutable records. Timeline fan-out is hybrid: fan out on write for ordinary authors, but merge pull-based feeds for accounts above a configurable follower threshold. A timeline cache stores ordered post IDs, not full mutable post objects.

### Timeline flow and correctness

On post commit, emit `PostCreated`; fan-out workers append the post ID to follower inboxes with bounded list length. A reader merges the cached inbox with celebrity pull results, removes deleted/blocked IDs, hydrates in batches, and returns a cursor based on `(event_time, post_id)` rather than wall-clock time alone. The invariant is that an acknowledged post is eventually visible to eligible followers and never appears after deletion or a block. At-least-once fan-out is safe because `(inbox_id, post_id)` is unique.

### API and storage sketch

`POST /v1/posts`, `GET /v1/timeline?cursor=...`, `POST /v1/follows/{user_id}`, and `POST /v1/posts/{id}/likes` use idempotency keys. Tables include `posts(post_id, author_id, body_ref, visibility, version)`, `follows(follower_id, followee_id, created_at)`, `timeline_items(owner_id, post_id, rank_key)`, and `engagement(post_id, actor_id, type)`. Index `(author_id, created_at, post_id)`, `(owner_id, rank_key)`, and unique engagement tuples. Search is an eventually consistent projection with tombstone handling.

### Ranking, abuse, and operations

Start chronological; introduce a ranking service later with feature snapshots and a bounded candidate set. Apply per-account posting limits, bot/anomaly detection, link and media scanning, block/mute enforcement, and privacy-aware deletion propagation. Monitor timeline freshness, fan-out lag, cache hit rate, celebrity spillover, moderation queue age, and p99 hydration latency. A hot celebrity should not create a write amplification storm: use pull-on-read and short-lived candidate caches.
