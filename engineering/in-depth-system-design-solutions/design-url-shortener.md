# IDS-004 · Design a URL shortener
...
Support create, redirect, custom aliases, expiration, optional destination edits, abuse review, and privacy-preserving click analytics. Assume 100 million new links/month, 10 billion redirects/month, 4,000 average and 40,000 peak redirects/second, 5-year retention for link metadata, and 90% of redirects to a small hot set.

### Architecture and ID strategy

The write service validates URLs, reserves aliases, scans destinations asynchronously, and writes a canonical link record. IDs can be generated from a database sequence encoded in base62, or from a collision-resistant random 64-bit token with a uniqueness constraint; choose sequence encoding when enumeration risk is acceptable, otherwise use random IDs plus abuse controls. Redirects hit an edge cache, then a regional cache, then a partitioned key-value store. Analytics events are buffered separately so the redirect response is not coupled to aggregation.

### API, schema, and flows

`POST /v1/links` with an idempotency key returns `201` and a short code; `GET /{code}` returns 301/302 based on cache and edit semantics; `PATCH /v1/links/{code}` uses `If-Match`; `DELETE` creates a tombstone. Store `links(code, owner_id, destination, status, expires_at, version, created_at)`, `aliases(owner_scope, alias, code)`, and `click_events(event_id, code, coarse_region, ua_class, occurred_at)`. Unique index `code`; secondary index `(owner_id, created_at)`; TTL only for expired derived analytics.

### Correctness, privacy, and abuse

Alias reservation is a transaction; a retry with the same key returns the original code. Redirect caches honor tombstones and short negative caching to avoid resurrection. Destination safety uses malware/phishing reputation, scheme allowlists, redirect-chain limits, and rate limits; provide a takedown workflow. Do not retain raw IP longer than policy requires; aggregate/coarsen location and user-agent. A delete must invalidate edge caches and be visible within a stated bound.

### Scale and follow-ups

Partition by code prefix with enough randomization to avoid hot ranges; replicate across AZs and asynchronously to a disaster-recovery region. Track redirect p99, cache hit ratio, stale/tombstone reads, creation collisions, scan backlog, and analytics lag. Discuss QR codes, custom domains, tenant isolation, link previews, GDPR deletion, and whether permanent redirects can ever be edited.
