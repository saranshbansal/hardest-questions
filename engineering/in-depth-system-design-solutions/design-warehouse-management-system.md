# IDS-013 · Design a Warehouse Management System
...
Design a WMS for inbound receiving, putaway, inventory locations, picking, packing, cycle counts, shipping, returns, labor tasks, and integrations with an OMS/carriers. Assume 1,000 warehouses, 100 million SKU-location balances, 50,000 concurrent handhelds, 20,000 picks/second globally at peak, and near-real-time stock visibility.

### Architecture and domain boundaries

Inventory ledger is the authoritative source; location, lot/serial, reservation, and movement services enforce physical constraints. Work orchestration assigns tasks to workers based on zone, priority, and capability. Handhelds use short-lived signed task leases and offline queues. Integrate receiving ASN, shipping labels, and carrier status through an adapter layer; do not let a carrier callback mutate stock without a validated movement.

### APIs and data

`POST /v1/receipts/{id}/confirm`, `POST /v1/movements`, `POST /v1/picks/{id}/complete`, `POST /v1/shipments`, and `GET /v1/inventory?sku=&location=` are idempotent. Model `locations`, `stock_lots(sku, location, quantity, reserved, version)`, `movements(event_id, from, to, quantity, reason)`, `tasks`, `workers`, and `holds`. Maintain a double-entry-like movement ledger and a materialized balance; unique event IDs prevent duplicate scans. Index queues by `(warehouse, zone, status, priority)` and stock by `(sku, warehouse, location)`.

### Invariants and offline operation

Quantity cannot become negative; every movement has a source/destination or an explicit adjustment reason; a serial number has one physical location; a pick consumes the reservation it was assigned. Reserve in the inventory partition with a version check. When a scanner is offline, accept only a bounded signed task batch, sequence scans locally, and reconcile conflicts at sync; do not blindly last-write-wins physical movement. Cycle counts create a variance workflow requiring approval for material adjustments.

### Reliability and evolution

Partition by warehouse and SKU, replicate within AZs, and asynchronously publish global availability. Backpressure inbound events rather than over-promise stock. Monitor scan-to-ledger latency, task lease expiry, negative-balance attempts, pick accuracy, queue age, inventory drift, and integration lag. Discuss robots, cold-chain constraints, lot expiry/FEFO, multi-owner inventory, and disaster-mode paper operations.
