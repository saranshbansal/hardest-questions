# IDS-007 · Design Zoom car app LLD
...
Design the core rental flow for a car-sharing app: vehicle search, availability, reservation, pickup, trip, extension, return, pricing, payment authorization, and incident handling. Keep external maps, payment, and IoT lock providers behind interfaces. The domain must prevent double booking while tolerating retries and delayed device callbacks.

### Domain objects and interfaces

```text
Vehicle { id, category, location, status, odometer }
Customer { id, license_status, payment_profile }
Reservation { id, customer_id, vehicle_id, start, end, state, version }
Trip { id, reservation_id, started_at, ended_at, state, odometer_start }

VehicleRepository.findAvailable(criteria)
ReservationRepository.reserve(command, expectedVersion)
LockProvider.unlock(vehicleId, idempotencyKey)
PricingPolicy.quote(vehicle, interval, customer)
PaymentGateway.authorize(amount, idempotencyKey)
```

Use dependency injection so pricing, lock, payment, and notification policies can be replaced in tests. A service object coordinates the transaction; entities enforce their own legal transitions.

### State machines and concurrency

Reservation states are `HELD -> CONFIRMED -> ACTIVE -> COMPLETED`; cancellation is allowed from held/confirmed with policy-based fees; expiration moves held to `EXPIRED`. Vehicle states are `AVAILABLE -> RESERVED -> IN_USE -> CLEANING -> AVAILABLE`, with `MAINTENANCE` as a fenced branch. Reserve in a transaction that locks or atomically claims `(vehicle_id, time_bucket)` rows and checks overlap. A unique reservation command key makes retries return the original result. Use optimistic `version` on edits; use a short lease only for a pickup lock operation.

```text
reserve(cmd):
  existing = commands.get(cmd.idempotencyKey)
  if existing: return existing.result
  tx:
    vehicle = vehicles.lock(cmd.vehicleId)
    assert vehicle.status == AVAILABLE
    assert reservations.noOverlap(cmd.vehicleId, cmd.start, cmd.end)
    hold = reservations.insert(..., state=HELD, version=1)
    commands.insert(cmd.key, hold.id)
  return hold
```

### APIs, persistence, and failure handling

`POST /v1/reservations`, `POST /v1/reservations/{id}/confirm`, `POST /v1/trips/{id}/start`, and `POST /v1/trips/{id}/end` require idempotency keys. Tables use unique `(vehicle_id, start, end, active_state)` via an exclusion/interval strategy where available, otherwise bucket rows plus a transaction. Payment authorization and IoT unlock are sagas: reservation is pending until authorization, and a failed unlock transitions to a recoverable incident rather than silently charging. Reconcile provider webhooks, expire abandoned holds, and audit every state transition.

### Testing and follow-ups

Test overlapping reservations under concurrency, duplicate webhooks, clock skew, lost mobile connectivity, partial payment capture, and a vehicle becoming unavailable after search. Ask whether pricing is fixed or dynamic, whether one-way trips are allowed, how late returns are billed, and which safety/identity checks are mandatory.
